  ...
  <div class="col-span-full flex flex-col gap-4 z-30 w-full">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        ...
        >
          {{ $t("whatsapp.customer.select_all") }}
        </button>
        <!-- Groupes de contacts -->
    ...
    <div class="relative">
      ...
      >
        ...
        <div v-if="isLoading" class="flex justify-center py-4">
          <div
            class="animate-spin rounded-full h-6 w-6 border-2 border-[#f3c775] border-t-transparent"
          ></div>
          <span>{{ $t("whatsapp.customer.loading") }}</span>
        </div>
          ...
          <div v-else class="space-y-1">
            ...
            >
              ...
              <p class="truncate text-sm">
                ...
                <span class="text-xs opacity-70">{{ contact.phone }} </span>
              </p>
            </div>
            <!-- Indicateur de chargement en bas -->
            <div v-if="loadingMore" class="flex justify-center py-2">
              <div
                class="animate-spin rounded-full h-4 w-4 border-2 border-[#f3c775] border-t-transparent"
              ></div>
              <span>{{ $t("whatsapp.customer.loading_more") }}</span>
            </div>
          </div>
        </template>
      </div>
...
const updateVisibleContacts = () => {
  ...
  const endIndex = Math.min(
    ...
    currentPage.value * PAGE_SIZE + BUFFER_SIZE
  );
